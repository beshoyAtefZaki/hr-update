...
def get_eos_reward(selected_emp=None, based_on_basic_salary=None):
    if selected_emp:
        ...
        if based_on_basic_salary == "1":
            ...
            #                                    order_by="-end_date")
            #
            latest_sal_struc = frappe.db.get_value("Salary Structure Employee",
                                                   {"employee": selected_emp, "docstatus": "0"}, "parent",
                                                   order_by="-to_date")
            ...
            earnings = frappe.db.get_values("Salary Detail",
                                            {"parent": latest_sal_struc, "docstatus": "0", "parentfield": "earnings"},
                                            "*")
...
def get_data_for_eval(selected_emp, salary_structure):
    ...
    data.update(frappe.get_doc("Salary Structure Employee",
                               {"employee": selected_emp, "parent": salary_structure}).as_dict())
