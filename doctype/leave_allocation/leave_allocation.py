	...
	def validate_leave_type(self):
		...
		if vacation_type == "Haj Vacation":
			date_of_joining = frappe.db.sql("""
			            ...
			            """, {'selected_emp': self.employee},as_dict=True)[0]

			worked_years = relativedelta(datetime.datetime.today(), date_of_joining['date_of_joining']).years

			if worked_years < 2 :
				frappe.throw(_("Haj Vacation Only for employees with more than two years of service"))
