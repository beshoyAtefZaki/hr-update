...
from frappe.utils import cint, cstr, date_diff, flt, formatdate, getdate, get_link_to_form, \
	comma_or, get_fullname,add_to_date,nowdate
# from erpnext.utils.utils import Overlab_Dates
		...
		if self.from_date and self.to_date:
			...
			if not is_lwp(self.leave_type):
				try:
					...
					if(vacation_type=="Sick Vacation"):
						self.leave_balance = get_leave_balance_on_sick_type(self.employee, self.leave_type,
							"{'from_date':'"+cstr(self.from_date)+"','to_date':'"+cstr(self.to_date)+"'}",
...
@frappe.whitelist()
def get_leave_balance_on_sick_type(employee, leave_type, date, allocation_records=None,
		consider_all_leaves_in_the_allocation_period=False):
	vacation_dates = frappe._dict()
	vacation_dates.update(ast.literal_eval(date))

	allocation_records = get_sick_leave_allocation_records(vacation_dates, employee,leave_type=leave_type).get(employee, frappe._dict())

	allocation = allocation_records.get(leave_type, frappe._dict())
	leaves_taken = get_approved_leaves_for_period(cstr(employee) , leave_type, allocation_records[leave_type]["from_date"], allocation_records[leave_type]["to_date"])
