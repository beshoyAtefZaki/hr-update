...
frappe.ui.form.on("Salary Slip", {
	setup: function(frm) {
		...
		frm.set_query("salary_component", "earnings", function() {
			return {
				...
				filters:[["type","=", "earning"],["salary_component_abbr","!=", "IN"]]
			}
		})
		frm.set_query("salary_component", "deductions", function() {
			return {

				filters:[["type","=", "deduction"],["salary_component_abbr","!=", "PN"]]
	...
	refresh: function(frm) {
		...
		cur_frm.fields_dict['insurance'].grid.set_column_disp(salary_detail_fields,false);
	},	
...
var calculate_earning_total = function(doc, dt, dn, reset_amount) {
	...
	for(var i = 0; i < tbl.length; i++){
		...
		if(!tbl[i].do_not_include_in_total) {
			...
			// if (tbl[i].salary_component == "Indemnity"){
			// 	IndemnityVal= tbl[i].amount;
			// 	var n = i;
			// 	frappe.model.get_value('HR Settings','=','indemnity_days_for_work_injuries',
			// 	  function(d) {
			// 		current_gross_pay = flt(total_earn)- flt(IndemnityVal) ;
			// 		tbl[n].amount = (current_gross_pay / 30.0) * flt(d.indemnity_days_for_work_injuries);
...
var calculate_ded_total = function(doc, dt, dn, reset_amount) {
	...
	refresh_many(['insurance', 'total_deduction']);
