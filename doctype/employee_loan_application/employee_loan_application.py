...
class EmployeeLoanApplication(Document):
	...
	def validate_loan_type(self):
		repayment_period_in_months = int(frappe.db.get_value('Loan Type', self.loan_type, 'repayment_period_in_months') or 0)
		if repayment_period_in_months and self.repayment_periods:
			if int(self.repayment_periods) > repayment_period_in_months :
				frappe.throw(_("Repayment Period in Months can not exceed {0}").format(repayment_period_in_months))
