...
def execute(filters=None):
	...
	for emp in sorted(att_map):
		...
		total_p = total_a = total_l=total_m=total_r = 0.0
		for day in range(filters["total_days_in_month"]):
			status = att_map.get(emp).get(day + 1, "None")
			status_map = {"Present": "P", "Absent": "A", "Half Day": "HD", "On Leave": "L","Mission":"M","Rest":"R", "None": "", "Holiday":"<b><font color=red>H</font></b>"}
		...
		row += [total_p, total_l, total_a,total_m,total_r]
...
def get_columns(filters):
	...
	columns += [_("Total Present") + ":Float:80", _("Total Leaves") + ":Float:80",  _("Total Absent") + ":Float:80",_("Total Missions") + ":Float:80",_("Total Rests") + ":Float:80"]
	return columns
